...
cap = cv2.VideoCapture(0)   # get 1 camera image, 0 is the name of the camera used (0,1,2,..)
cap.set(cv2.CAP_PROP_FRAME_WIDTH, 1280) #Get HD footage
cap.set(cv2.CAP_PROP_FRAME_HEIGHT, 720) #Get HD footage

while True :    #Get it real time (analyze for every frame)
    ...
    #Note that if multiple markers are detected, 'corners' becomes a list with each marker accessible via
        ...
        cv2.waitKey(1)              #in a 1ms window (put 0 for infinite)

    if(cv2.waitKey(1) == ord("q")): #Exit loop if presing "q" onkeyboard
            break
